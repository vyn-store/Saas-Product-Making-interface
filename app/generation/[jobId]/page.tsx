      ...
      <div className="max-w-4xl mx-auto px-4 md:px-8 pb-16">
        {isComplete ? (
          <div className="space-y-8 animate-scale-in">
            {/* Error/Warning Card for failures */}
            {results?.error && (
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h2 className="font-display text-2xl font-bold text-[var(--text-primary)] mb-3">
                      {results.imageUrl || results.videoUrl ? 'Partial Generation Failure' : 'Generation Failed'}
                    </h2>
                    <p className="text-sm text-[var(--text-secondary)] leading-relaxed mb-4">
                      <span className="font-semibold text-red-400">Error:</span> {results.error}
                    </p>
                    {(results.imageUrl || results.videoUrl) && (
                      <p className="text-sm text-[var(--text-secondary)] leading-relaxed">
                        {results.imageUrl && !results.videoUrl && 'The image was generated successfully, but video generation failed.'}
                        {!results.imageUrl && results.videoUrl && 'The video was generated successfully, but image generation failed.'}
                      </p>
                    )}
                  </div>
                </div>
